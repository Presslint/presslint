...
use crate::extgstate_classify::{
    ClassifiedExtGStateResource, SkippedExtGStateResource,
    inspect_effective_extgstate_resource_entries,
};
...
/// Per-document page `/Resources /ExtGState` classification report.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DocumentPageExtGStateResourcesInspection {
    /// Total source length supplied by the caller.
    pub byte_len: usize,
    /// Document-ordered per-page `ExtGState` resource reports.
    pub pages: Vec<PageExtGStateResourcesInspection>,
...
impl DocumentPageExtGStateResourcesInspection {
...
/// Per-page classified page-scope `ExtGState` resources.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PageExtGStateResourcesInspection {
    ...
    /// Page-local structural `ExtGState` diagnostics.
    pub skipped: Vec<SkippedExtGStateResource>,
}

/// Error returned when page `ExtGState` resource inspection cannot begin.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DocumentPageExtGStateResourcesInspectionError {
...
/// Inspect page-scope `/Resources /ExtGState` entries through a classic xref
/// table.
///
/// # Errors
///
...
pub fn inspect_document_page_extgstate_resources(
    input: &[u8],
...
/// Inspect page-scope `/Resources /ExtGState` entries through any object lookup
/// backend.
///
/// The walk follows the page tree root-down in document order, carrying the
/// effective inheritable `/Resources` dictionary. Each `/ExtGState` entry is
/// classified shallowly into Phase-1 safety parameters; unsupported or
/// unresolved resource shapes become structured page-local skips.
///
/// # Errors
///
...
/// successful report.
pub fn inspect_document_page_extgstate_resources_with_lookup(
    input: &[u8],
...
) -> Result<DocumentPageExtGStateResourcesInspection, DocumentPageExtGStateResourcesInspectionError>
{
    let root_targets =
        crate::inspect_page_tree_kid_targets_with_lookup(input, lookup, root_node_object_offset)
            .map_err(|error| DocumentPageExtGStateResourcesInspectionError {
    ...
    let mut walk = ExtGStateResourceWalk::new();
    walk.visited.insert(
        root_targets
            .kids
            .node
    ...
    Ok(DocumentPageExtGStateResourcesInspection {
        byte_len: input.len(),
        pages: walk.pages,
...
struct ExtGStateResourceWalk {
    pages: Vec<PageExtGStateResourcesInspection>,
...
impl ExtGStateResourceWalk {
    ...
    fn inspect_page(
        ...
        let mut page =
            inspect_effective_extgstates(input, lookup, page_object_byte_offset, &context);
...
fn inspect_effective_extgstates(
    ...
    let effective = inspect_effective_extgstate_resource_entries(
        input,
        lookup,
